...
@Component({
  ...
  styleUrl: './add-sms-template.component.css'
})
export class AddSmsTemplateComponent implements OnInit {
  ...
  this.canEdit = this.isSuperAdmin || perms.includes('edit_sms_template');
  ...
  load() {
    this.loader.start();
    this.service.getSmsTemplateById(this.id).subscribe({
      next: (res: any) => {
 ...
 enableEdit() {
  // 👇 NAYA — permission check, category component jaisa
  if (!this.canEdit) {
    this.toastr.error('Access Denied: You cannot edit sms templates.');
    return;
  }
  ...
  cancel() {
    this.router.navigate(['/manage-sms-templates']);
  }
  ...
  save() {
    ...
    const req = this.isEditMode
      ? this.service.updateSmsTemplate(requestData)
      : this.service.createSmsTemplate(requestData);
